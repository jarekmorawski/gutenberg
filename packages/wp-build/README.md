...
This is useful in monorepos where multiple WordPress plugins share an internal package that lives outside any single plugin's directory. Each entry points to the directory that contains the shared packages — it does not need to be named `packages/`:
